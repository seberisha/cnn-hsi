...
def build_net(network, X, Y, num_classes, num_epochs, checkpoint_path, size_batch, Xval=None, Yval=None, dec_step=100,
    ...
    ###########################################################################
    network = fully_connected(network, 512, weights_init=nor, regularizer='L2', weight_decay=0.001)
    # network = batch_normalization(network)
    network = tflearn.activations.softplus(network)
    if train:
        network = dropout(network, 0.5)

    network = fully_connected(network, num_classes, activation='softmax', weights_init=nor)
